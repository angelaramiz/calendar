...
- Known issues (updated Sep 2026):
  ...
  - Bottom tabs (`FinTrackBottomBar`: Inicio/Calendario/Flujos/Presupuesto/Cuentas) navigate with `popUpTo + launchSingleTop` (helper `navigateToTab` in `NavGraph.kt`).
  - Presupuesto tab (`ui/budget/`, análisis financiero: corto/mediano/largo plazo + topes + metas) no longer hosts money admin — that lives in Cuentas tab (`ui/accounts/` + `AccountsViewModel`): débito (`WalletStore`), crédito (`CreditCardStore`), servicios (`ServiceBillStore`), suscripciones (`SubscriptionDetector` + crear recurrente). Permisos no longer has Mis billeteras.
  ...
  - Flows tab (`ui/flows/` + `domain/FlowEngine`): lienzo estilo n8n (`FlowsCanvas.kt`: nodos con puertos, conectores curvos con montos, ramas Sí/No, tap abre editor en bottom sheet) + traza de ejecución (`FlowTrace`: entrada/salida por nodo y rama tomada). Config persiste en DataStore (`data/FlowStore.kt`) as JSON — no new Supabase tables.
  ...
  - Planificar compra (`ui/product/ProductShareScreen.kt` + `domain/ProductScraper` + `data/remote/ProductScraperRepository.kt`): recibe links compartidos (ACTION_SEND text/plain en `MainActivity` → ruta `share_product?url=`) de Mercado Libre/Amazon, consulta `POST /api/scrape/quick` al backend Fly.io+Selenium y crea la meta con nombre/precio/tienda/link (`SavingsGoal.url/store`, compatibles con metas viejas). Parser defensivo (platform puede venir objeto); CAPTCHA/bloqueo → alta manual. Entrada también desde botón "Planificar compra con link" en Objetivos; `BudgetViewModel` compartido por actividad.  - JVM unit tests: `app/src/test/` (JUnit4); run `:app:testReleaseUnitTest --tests "<Clase>"`. `PatternExpanderTest` 10/10, `NotificationParserTest` 25/25, `MonthSummaryTest` 7/7, `FlowEngineTest` 21/21, `BudgetPlannerTest` 15/15, `GoalPlannerTest` 15/15, `PendingTxCodecTest` 8/8, `PendingOpCodecTest` 5/5, `WalletResolverTest` 9/9, `SubscriptionDetectorTest` 7/7, `CreditCardPlannerTest` 8/8, `ServiceBillsTest` 8/8, `AuthErrorsTest` 8/8, `PatternValidatorTest` 8/8, `TransactionDayTest` 4/4, `DedupTest` 7/7, `LocalBackupTest` 5/5, `TransactionCategoriesTest` 3/3 must stay green.
  ...
  - Inicio tiene pull-to-refresh propio (sin librerías): arrastrar desde el tope estira la lista (`NestedScrollConnection` + `translationY`) y al soltar dispara `DashboardViewModel.refreshAll()` (recarga + cola pendiente + OTA silencioso).
- Sesión persistente: `SupabaseClientProvider` usa `alwaysAutoRefresh/autoLoad/autoSave=true` y `enableLifecycleCallbacks=false` (intencional: con true el refresco se pausa sin foco y el listener en segundo plano expiraba). `AuthRepository.ensureSession()` intenta `refreshCurrentSession()` antes de rendirse; el listener y la huella la usan.
  ...
  - Billeteras 100% locales (`data/WalletStore.kt` + `domain/WalletResolver`): Efectivo + una por banco con regla por paquete origen; overrides `tx:<id>`/`mov:<id>`; formularios sin selector de billetera (solo Método de pago = tag de tarjeta). Sin columna en el servidor (sin acceso DDL). Las fijas vienen por defecto; el usuario agrega/quita las suyas en Permisos → Mis billeteras o con "+ Nueva" en los formularios (`WalletRow.custom`).
  ...
  - Pagos de tarjeta: `CardPayment` (monto real contra un corte ISO) descuenta del restante; botón "Marcar pago" con estimado editable. Recordatorio 3/1/0 días vía `RemindersWorker` (salta si ya hay pago contra ese corte).
  - Servicios (`data/ServiceBillStore.kt` + `domain/ServiceBills`): altas con monto estimado, día de vencimiento y monthly/bimonthly; sección en Presupuesto; recordatorio 3/1/0 días. Distinto de suscripciones (monto variable, importa el vencimiento).
  - Recurrentes clasificados (`data/PatternLinkStore.kt`): Normal/Tarjeta/Servicio/Suscripción por patrón (local, `PatternLinkKind`); confirmar un recurrente de tarjeta tagea el movimiento; vencimientos y pagos salen como marcadores en el calendario (`markers`, sin crear movimientos).
  - Recordatorios: `data/service/RemindersWorker.kt` (WorkManager, dos pasadas diarias 12:00 y 22:00, `RemindersNotifier` canal `fintrack_recordatorios`, dedup por turno en `ReminderStore`) cubre servicios, pagos de tarjeta y eventos del calendario de hoy (estos últimos requieren sesión).
...
- OTA flow: `OtaUpdateRepository.checkForUpdate()` compares remote `versionCode` vs `BuildConfig.VERSION_CODE`; `DashboardViewModel` exposes `updateAvailable`; `DashboardScreen` shows dialog; `OtaInstaller` downloads via `DownloadManager` and installs via `FileProvider` (`res/xml/file_paths.xml`, authorities `${applicationId}.fileprovider`). Requires `REQUEST_INSTALL_PACKAGES` + user granting "install unknown apps". `release.ps1` publishes `apkSha256` (in `version.json` + Supabase `valor`); the app verifies SHA-256 before installing (fail-closed: mismatch or missing hash = deleted + warning, never installed).
...
- Skill reutilizable por cualquier agente: `.agents/skill/emulador-android/` (`SKILL.md` + `emu.ps1`: `status|boot|install|launch|shot|dump|tap|type|swipe|key|tile|panel|net`) — adb directo, sin Bun.
